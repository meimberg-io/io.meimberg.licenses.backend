...
| `APP_DOMAIN` | Domain for the application | `licenses-backend.meimberg.io` |
| `DB_USERNAME` | Database username (optional, defaults to `licenses`) | `licenses` |
...
| `DB_PASSWORD` | Database password | Secure password (generate with `openssl rand -base64 32`) |
...
```
licenses-backend.meimberg.io  A  <server-ip>
```
...
4. Verify application is accessible: `https://licenses-backend.meimberg.io/actuator/health`

**Note:** The `.env` file is automatically created on the server during deployment. No manual server setup required.
...
- Write access to `/srv/projects/licenses-backend`
